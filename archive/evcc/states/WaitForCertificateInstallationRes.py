#######################################################
# 
# WaitForCertificateInstallationRes.py
# Python implementation of the Class WaitForCertificateInstallationRes
# Generated by Enterprise Architect
# Created on:      07-Jan-2021 11:13:57
...
class WaitForCertificateInstallationRes(ClientState):
    ...
    def process_incoming_message(self, message) -> ReactionToIncomingMessage:
        ...
        """
        if self.is_incoming_message_valid(message, CertificateInstallationResType.__class__):
            v2g_message_res: V2GMessage = message
            certificate_installation_res: CertificateInstallationResType = \
                v2g_message_res.get_body().get_body_element().get_value()

            if not self.__verify_signature(certificate_installation_res, v2g_message_res.get_header().get_signature()):
            ...
            cert_chain_response_code: ResponseCodeType = SecurityUtils.verify_certificate_chain(
                certificate_installation_res.get_sa_provisioning_certificate_chain(),
                str(GlobalValues.EVCC_TRUSTSTORE_FILEPATH), PKI.CPS)

            if not cert_chain_response_code == ResponseCodeType.OK:
                return TerminateSession("Provisioning certificate chain is not valid")

            oem_prov_cert_private_key = SecurityUtils.get_private_key(SecurityUtils.get_keystore(
            ...
            if not SecurityUtils.save_contract_certificate_chain(
                    ...
                    certificate_installation_res.get_contract_signature_cert_chain(),
                    SecurityUtils.decrypt_contract_cert_private_key(
                        certificate_installation_res.get_dh_public_key().get_value(),
                        certificate_installation_res.get_contract_signature_encrypted_private_key().get_value,
                        oem_prov_cert_private_key
                    )):
    ...
    def __verify_signature(self, certificate_installation_res: CertificateInstallationResType,
                           signature: SignatureType) -> bool:
        """
        Creates dict verify_xml_sig_ref_elements with relevant signature data and initiates verification of signature.
        :param certificate_installation_res: CertificateInstallationResType
        ...
        verify_xml_sig_ref_elements = {
            certificate_installation_res.get_contract_signature_cert_chain().get_id(): SecurityUtils.generate_digest(
                certificate_installation_res.get_contract_signature_cert_chain().get_id(),
                self.get_message_handler().get_jaxb_element(
                    certificate_installation_res.get_contract_signature_cert_chain())),
            certificate_installation_res.get_contract_signature_encrypted_private_key().get_id():
                SecurityUtils.generate_digest(
                    certificate_installation_res.get_contract_signature_encrypted_private_key().get_id(),
                    self.get_message_handler().get_jaxb_element(
                        certificate_installation_res.get_contract_signature_encrypted_private_key())),
            certificate_installation_res.get_dh_public_key().get_id(): SecurityUtils.generate_digest(
                certificate_installation_res.get_dh_public_key().get_id(),
                self.get_message_handler().get_jaxb_element(certificate_installation_res.get_dh_public_key())),
            certificate_installation_res.get_emaid().get_id(): SecurityUtils.generate_digest(
                certificate_installation_res.get_emaid().get_id(),
                self.get_message_handler().get_jaxb_element(certificate_installation_res.get_emaid()))}

        if not SecurityUtils.verify_signature(
                signature,
                self.get_message_handler().get_jaxb_element(signature.get_signed_info()),
                verify_xml_sig_ref_elements,
                certificate_installation_res.get_sa_provisioning_certificate_chain().get_certificate()):
            return False
